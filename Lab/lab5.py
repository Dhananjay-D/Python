...
class Customer:
    ...
    def placeOrder(self, order):
        # order is an object.Add the given order to orders list
...
for i in range(noOfCusts):
    name, email = input(
        'Enter name and email for Customer '+str(i+1)+': ').split()
    ...
    for j in range(noOfOrders):
        ...
        Prods = []
        noOfProds = int(input('Enter no of products for order ' +
                        str(oId)+' for Customer '+name+': '))
...
for cust in Custs:
    ...
    orders = cust.getOrders()   #list of orders
    for order in orders:
